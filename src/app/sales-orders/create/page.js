...
export default function CreateSOPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!customerName || isInvalid) {
            toast.warning("Please complete customer name and all item fields.");
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            if (response.status === 403) {
                toast.error("You are not authorized to create sales orders.");
                return;
            }
            ...
            if (response.status === 400) {
                toast.warning(data.error || "Invalid sales order data.");
                return;
            }
    ...
    const addItem = () =>
        setItems([...items, { productId: "", variantId: "", orderedQty: 1, unitPrice: 0 }]);
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 p-8 transition-colors">
            <form className="max-w-4xl mx-auto space-y-6" onSubmit={handleSubmit}>
                ...
                <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md p-6 shadow-sm">
                    ...
                    <input
                        type="text"
                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-md px-4 py-4 focus:border-blue-500 outline-none"
                    ...
                    <div className="p-6 space-y-4">
                        {items.map((item, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className="grid grid-cols-1 md:grid-cols-12 gap-4 bg-slate-50 dark:bg-slate-950/50 p-5 rounded-md border border-slate-200 dark:border-slate-800 relative"
                                >
                                    <div className="md:col-span-4">
                                        ...
                                        <select
                                            className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md px-3 py-3 text-sm outline-none"
                                    ...
                                    <div className="md:col-span-3">
                                        ...
                                        >
                                            ...
                                            {product?.variants.map(v => (
                                                <option key={v._id} value={v._id}>{v.sku}</option>
                                            ))}
                                        </select>
                                    </div>
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <input
                                            type="number"
                                            className="w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-md px-3 py-3 text-sm text-slate-500 cursor-not-allowed"
                ...
                <div className="flex justify-end pt-4">
                    <button
                        type="submit"
                        disabled={loading}
                        className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-10 rounded-md disabled:opacity-50 transition-all"
                    >
